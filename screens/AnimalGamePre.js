...
import data from '../Data/dataAnimalGamePre';
...
const AnimalGamePre = () => {
  ...
  const handleNextClick = () => {
    ...
    if (nextQuestion < shuffledData.length) {
      ...
      navigation.navigate('score', { score: scoreRef.current, totalQuestions: shuffledData.length, screenName: 'Toddler Animal Game', });
    }
  };
  ...
  const storeScore = async (score, totalQuestions, screenName) => {
    try {
      const achievementData = { score, totalQuestions, screenName };
      await AsyncStorage.setItem(`achievementData_${screenName}`, JSON.stringify(achievementData));
    } catch (error) {
